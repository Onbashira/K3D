...
HRESULT K3D::DepthStencil::Create(UINT width, UINT height, DXGI_FORMAT resourceFormat, DXGI_FORMAT clearValueFormat)
{
	...
	Framework::GetDevice().GetDevice()->CreateDepthStencilView(GetResource(), &dsvDesc, _dsvHeap.GetCPUHandle(0));
...
HRESULT K3D::DepthStencil::CreateView(D3D12_SHADER_RESOURCE_VIEW_DESC srv, D3D12_CPU_DESCRIPTOR_HANDLE handle)
{
	...
	Framework::GetDevice().GetDevice()->CreateShaderResourceView(this->GetResource(), &srv, handle);
	return S_OK;
}

void K3D::DepthStencil::Discard()
{
...
void K3D::DepthStencil::ClearDepthStencil(std::shared_ptr<K3D::CommandList> list)
{
