...
K3D::StructuredBuffer::StructuredBuffer()
{
}
...
HRESULT K3D::StructuredBuffer::CreateDescriptors(unsigned int elementSize, unsigned int numElements)
{
	//UAVStructuredView
	{
...
D3D12_CPU_DESCRIPTOR_HANDLE K3D::StructuredBuffer::GetSRVCPUHandle()
{
	return _heap.GetCPUHandle(HEAP_OFFSET::SRV_DESCRIPTOR_OFFSET);
}

D3D12_CPU_DESCRIPTOR_HANDLE K3D::StructuredBuffer::GetUAVCPUHandle()
{
	return _heap.GetCPUHandle(HEAP_OFFSET::UAV_DESCRIPTOR_OFFSET);
}

D3D12_GPU_DESCRIPTOR_HANDLE K3D::StructuredBuffer::GetSRVGPUHandle()
{
	return _heap.GetGPUHandle(HEAP_OFFSET::SRV_DESCRIPTOR_OFFSET);
}

D3D12_GPU_DESCRIPTOR_HANDLE K3D::StructuredBuffer::GetUAVGPUHandle()
{
...
HRESULT K3D::StructuredBuffer::CreateView(D3D12_UNORDERED_ACCESS_VIEW_DESC * uavDesc, D3D12_CPU_DESCRIPTOR_HANDLE cpuDescriptorHandle, Resource * counterResource)
{
	...
	Framework::GetDevice().GetDevice()->CreateUnorderedAccessView(_resource.Get(), counterResource == nullptr ?  nullptr: counterResource->GetResource(), uavDesc, cpuDescriptorHandle);
	return S_OK;
}
...
void K3D::StructuredBuffer::WriteToBuffer(unsigned int numElements, unsigned int elementSize, void * pBufferData)
{
...
K3D::DescriptorHeap * K3D::StructuredBuffer::GetHeap()
{
	return &_heap;
}

D3D12_UNORDERED_ACCESS_VIEW_DESC K3D::StructuredBuffer::GetUAVDesc()
{
	return _unorderedAccessViewDesc;
}

D3D12_SHADER_RESOURCE_VIEW_DESC K3D::StructuredBuffer::GetSRVDesc()
{
