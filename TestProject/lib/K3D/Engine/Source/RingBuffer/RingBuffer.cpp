...
K3D::RingBuffer::RingBuffer(unsigned int maxSize, IMemoryAllocator& allocator) noexcept :
...
K3D::RingBuffer::RingBuffer(RingBuffer && buffer) noexcept :
...
K3D::RingBuffer & K3D::RingBuffer::operator=(RingBuffer && buffer) noexcept
{
...
K3D::RingBuffer::~RingBuffer()
{
	Discard();
}

void K3D::RingBuffer::Discard()
{
...
unsigned int K3D::RingBuffer::Allocate(unsigned int size, unsigned int alignment)
{
	if (size <= 0) {
...
void K3D::RingBuffer::CompleteCurrentFrame(unsigned long fenceValue)
{
	if (_currentSize != 0) {
...
void K3D::RingBuffer::ReleaseCompletedFrame(unsigned long completedFenceValue)
{
