	...
	for (UINT i = 0; i < paths.size(); i++) {
		if (managerRef._textureResourceMap.GetMap().find(paths[i]) == managerRef._textureResourceMap.GetMap().end()) {
			...
			else if (fp != NULL)
			{
				...
				LoadRawTexture(commandList, commandQueue, resource, paths[i]);
				//file Close
				fclose(fp);
...
HRESULT K3D::TextureLoader::LoadRawTexture(std::shared_ptr<CommandList> commandList, CommandQueue * commandQueue, std::shared_ptr<ShaderResource>& resource, std::string path)
{
	...
	hr = UpdateSubResource(Framework::GetCommandList(), &Framework::GetCommandQueue(), resource, desc.subResource, desc.fileName);
...
HRESULT K3D::TextureLoader::LoadRawTexture(std::shared_ptr<CommandList> commandList, CommandQueue * commandQueue, std::shared_ptr<TextureObject>& textureObj, std::string path)
{
	auto& device = Framework::GetDevice()->GetDevice();
	...
	textureObj.reset();
	textureObj = std::make_shared<TextureObject>()

	hr = UpdateSubResource(Framework::GetCommandList(), &Framework::GetCommandQueue(), textureObj->GetShaderResource(), desc.subResource, desc.fileName);
